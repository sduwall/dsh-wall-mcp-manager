window.__ModuleLoader__.load({
  id: '@sduwall/dsh-wall-mcp-manager',
  factory: (require) => {
    var module = { exports: {} }
     ...
     * 把一条提示并入队列。
     *
     * 用 `channel` 归并而不是无脑追加：`watchServer` 每轮轮询都会改写同一个
     * 服务的提示文案（正在连接… → 已连接），追加的话 8 秒内就能堆出十几条
     * 同源消息。同 channel 原地更新且保留原 `id`，销毁计时的身份才不会被
     * 文案变化重置；不同 channel（不同服务、不同操作）则各自排队共存。
     *
     ...
     * `secrets` 只告诉我们「这个服务有没有凭据」，不含任何键名——
     * 这正是我们要的：连键名都不出网。
     */
     ...
     * 把表单草稿翻成 mutate 操作序列。
     *
     * 逐字段寻址（`servers.<name>.<field>`）而不是整体替换该服务：
    ...
    function draftToOps(name, draft, isNew) {
      if (!SERVER_NAME_PATTERN.test(name)) {
        return { ok: false, message: '服务名称需为 1-32 位的字母、数字、下划线或连字符' }
      }
      ...
      const ops = []
      if (isNew) ops.push({ op: 'set', path: at('enabled'), value: false })
    ...
    function McpManagerSection() {
       ...
       * 待确认删除的 serverName。
       *
       * 删除会连带抹掉该服务的凭据（`secretEnv` / `secretHeaders`），而凭据
       ...
       * 用服务名作 channel 且标为 `pending`，好让轮询原地把「正在连接…」改写
      ...
      const watchServer = async (serverName) => {
        ...
        pushNotice({
          channel: serverName,
          kind: 'error',
          text: `${serverName} 仍未注册任何工具。实例已启动，但与 MCP 服务的握手尚未成功（mcp-client 会在后台继续重连）；请检查地址/命令是否正确，详情见 DSH 日志。`,
        })
      }
            ...
            ? h(
                ...
                tools.length === 0
                  ? h(
                      ...
                      { style: { ...hintStyle, marginTop: 0 } },
                      status?.mounted === true
                        ? '该服务尚未注册任何工具：实例已启动，但与 MCP 服务的握手未完成。可能仍在连接中（点「刷新状态」重看），也可能是地址/命令有误——mcp-client 会在后台按退避重连，失败原因只在 DSH 日志里。'
                        : '该服务未挂载，无法列出工具。',
                    )
      ...
      return h(
        ...
        h(
          ...
          '集中管理本机 DSH 连接的 MCP 服务，并查看每个服务提供的工具、调用参数与返回内容。',
          '配置保存后立即生效：新增会即时连接，修改会重连该服务，删除会断开连接，均无需重启 DSH。',
        ),
        config.writable
          ? undefined
          : h(
        ...
        editing === undefined
          ? undefined
          : h(ServerForm, {
              // key 让「编辑另一个」时表单状态整体重建，而不是残留上一个的草稿
        ...
        names.length === 0
          ? h(
              'p',
              { style: { ...hintStyle, marginTop: 0 } },
              '还没有配置任何 MCP 服务。点击「添加服务」开始，例如用 stdio 方式运行 ',
        ...
        runtime.orphans.length === 0
          ...
          : h(
              ...
              h(
                ...
                '这些工具由本插件之外的 mcp-client 实例注册（例如部署配置里直接挂载的服务）。',
