...
/**
 * 硬依赖两个服务：
 * - `loader`：本插件的全部功能建立在「运行时把每台 MCP 服务器挂成一个
 ...
 * @property {Record<string, object>} servers MCP 服务器清单
 */
...
export async function apply(ctx, config) {
  ...
  async function mount(serverName, plan) {
    ...
    ctx.logger.info(`[dsh-wall-mcp-manager] 已挂载 MCP 服务器 ${serverName}`)
  }

  /** 卸载一台服务器；entry 可能已被组合层移走，故先探 store 再 remove */
  async function unmount(serverName) {
    ...
    ctx.logger.info(`[dsh-wall-mcp-manager] 已卸载 MCP 服务器 ${serverName}`)
  }

  /**
   * 对账一次：让实际挂载的实例集合收敛到配置描述的集合。
   *
   * 单台服务器挂载失败（命令不存在、serverName 撞车、远端不可达且
   * failOnStartupError=true）只记进 `failures` 交给界面显示，绝不外溢——
   * 一台配错的服务器不该让插件整体卸载，也不该拖垮其他服务器。
   */
  async function reconcile() {
    ...
    for (const [serverName, plan] of desired) {
      ...
      try {
        ...
        record.key = plan.key
        ctx.logger.info(`[dsh-wall-mcp-manager] 已更新 MCP 服务器 ${serverName} 的配置`)
      } catch (error) {
        ...
        ctx.logger.warn(
          `[dsh-wall-mcp-manager] MCP 服务器 ${serverName} 挂载失败：${errorText(error)}`,
        )
      }
    }
  ...
  /**
   * 汇总界面需要的服务器视图：配置里有什么、实际挂上了没有、fiber 处于哪一相。
   *
  ...
  /** 工具清单：按已配置的服务器归组，另外单列不属于任何配置项的 mcp 工具 */
    ...
    queue = queue.then(async () => {
      for (const serverName of [...mounted.keys()].reverse()) {
        try {
          ...
          ctx.logger.warn(
            `[dsh-wall-mcp-manager] 卸载 MCP 服务器 ${serverName} 失败：${errorText(error)}`,
          )
        }
      }
    })
